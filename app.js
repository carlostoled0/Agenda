  ...
  try {
    ...
    if (jsonDataMatch && jsonDataMatch[1]) {
      try {
        ...
        console.warn('JSON-ld mal formatado, tentando próximo método...');
      }
    }

    const manualMatch = html.match(/"channelId":"(UC[\w-]{22})"/);
    if (manualMatch && manualMatch[1]) {
      return manualMatch[1];
    }

    const metaOgUrlMatch = html.match(/<meta property="og:url" content="https:\/\/www\.youtube\.com\/channel\/(UC[\w-]{22})"/);
    if (metaOgUrlMatch && metaOgUrlMatch[1]) {
      return metaOgUrlMatch[1];
    }
    ...
    throw new Error('❗ Canal não localizado automaticamente. Utilize o ID do canal manualmente.');
  } catch (error) {
    console.error(error);
...
document.getElementById('btnBuscar').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!channelId) throw new Error('❗ ID do canal não encontrado. Verifique o link.');
    ...
    if (data.items && data.items.length > 0) {
      ...
      throw new Error('❗ Canal encontrado mas sem dados públicos.');
    }
  } catch (error) {
    console.error(error);
